...
rateSet = {'const-const', '2phase-const', 'linear'};
...
% Artificial modification for 2 phase
if rateID == 2
    xmax(3) = x(3);
    xmin(3) = x(3);
end
...
if useParadis
    if rateID == 1
        ...
        % Time varying Paradis estimation
        wantPlot = 1;
        nPtperVar = 30;
...
for i = 1:nData
    ...
    [tsol, qsol] = ode113(@(ts, y) odeSnyBDTrapz(ts, y, xsetMx, numRV, Tsp, nLinCurr, inttT, lamt),...
    ...
    pert = getNeeRatesTrapz(tsol(end), xsetMx, numRV, Tsp, nLinCurr, inttT, lamt);
    qev(i+1, :) = qsol(end, :);
    tev(i+1) = tsol(end);
