...
% Create space around true parameters
minSpace = [0.1 0.1 0.01];
maxSpace = [10 20 0.1];
...
for i = 1:nT
    ...
    PTn(i) = ((1 - P0Ts*rst(0, Ti, x)).^(n1-1)).*(P0Ts^2)*rst(0, Ti, x);
end
...
for i = 1:nData
    ...
    [tsol, qsol] = ode113(@(ts, y) odeSnyIntVary(ts, y, xsetMx, numRV, Tsp, nLinCurr),...
    ...
    lampert = getNeeTimeVaryInt(tsol(end), xsetMx, numRV, Tsp, nLinCurr);
    qev(i+1, :) = qsol(end, :);
    tev(i+1) = tsol(end);
...
figure;
plotyy(tn, [lamt lamhat], tn, [mut muhat]);
xlabel('time');
grid;
