...
function ratediag = getNeeTimeVarySimpleNee(t, xsetMx, numRV, Tsp, ncurr)
...
PtT = zeros(size(y{1}));
rst = @(t1, t2, lam, mu) exp((mu-lam)*(t2-t1));
intT = @(t1, t2, lam, mu) mu*rst(t1, t2, lam, mu);

% Loop across parameter space
for k = 1:length(y{1})
    % Nee
    PtT(k) = (1 + integral(@(t2)intT(t, t2, lam1(k), mu1(k)), t, Tsp))^(-1);
end
