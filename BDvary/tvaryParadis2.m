...
function estParadis = tvaryParadis2(xmin, xmax, nPtperVar, numRV, n, tspec, tplotcdf, T, lamt,...
    rhotT, inttT, wantPlot, x, xdefs, tcdf)

% Assumptions and modifications
% - added integral of net rate to compare sum of square errors
...
% Storage of square errors and cdfs and net rate lamt - mut
ss_err = zeros(1, m);
rateInt = zeros(1, m);
cdf_p = cell(1, 1);
nInt = 200; % for integration
...
for k = 1:m
    ...
    % Get lamt - mut integral across range
    rateInt(k) = -rhotT(xk, 0, T);
...
% Marginalise the sum of square errors (pseudo-prob like)
[ss_marg, ~] = marginalise(numRV, IDMx, ss_err, nPtperVar*ones(1, numRV));
...
if wantPlot
    ...
    % Plot sum of squares across m combinations and marginalised
    figure;
    plot(1:m, ss_err);
    xlabel('points');
    ...
    for i = 1:numRV
        ...
        plot(xset{i}, ss_marg{i}, 'b', [x(i) x(i)], [0 max(ss_marg{i})], 'k',...
            [estParadis.xhat(i) estParadis.xhat(i)], [0 max(ss_marg{i})], 'r');
        xlabel(['x_' num2str(i)]);
        ylabel('sum of square errors');
        legend('marginal', 'true value', 'estimate', 'location', 'best');
        grid;
    end
