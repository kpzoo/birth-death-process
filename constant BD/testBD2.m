...
% - allows for critical lam = mu case
...
% Initialise Snyder filtering problem for estimation of [rho sig] or mu
n = 100;
if ~critBD
...
if ~critBD
    ...
    % Parameter for critical birth-death
    mu = x;
end

% Simulate the constant or critical BD process according to Hartmann 2008
if ~critBD
    [T, tbranch] = genBirDeaConst(n, sig, rho);
else
    [T, tbranch] = genBirDeaCritical(n, mu);
end
...
if ~critBD
    ...
    for i = 1:nData
        ...
        % Solve linear ODEs continuously with setting of options, no Q
        [tsol, qsol] = ode113(@(ts, y) odeSnyBDcrit(ts, y, xsetMx, numRV, Tsp, nLinCurr),...
            [tspec(i) tspec(i+1)], qev(i, :)', options);
        ...
        % Perturb the q posterior for the new event
        lampert = getNeeRateCrit(tsol(end), xsetMx, numRV, Tsp, nLinCurr);
        qev(i+1, :) = qsol(end, :);
        tev(i+1) = tsol(end);
        qev(i+1, :) = qev(i+1, :).*lampert./(qev(i+1, :)*lampert');
